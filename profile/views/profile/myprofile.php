<div id="profile">
    ...
    <table class="dataGrid">
        ...
        <tr>
            ...
            <td><?php
                echo CHtml::link(Yum::t('Change password'), array(
            ...
            foreach ($friends as $friend) {
                ...
                <div id="friend">
                    <div id="avatar">
                        ...
                        <div id='user'>
                            <?php
                            echo CHtml::link(ucwords($friend->username), Yii::app()->createUrl('profile/profile/view', array(
                                        'id' => $friend->id)));
                            ?>
                        </div>
                    </div>
...
<div id="visits">
    ...
    if ($model->visits) {
        ...
        echo '<table>';
        printf('<th>%s</th><th>%s</th><th>%s</th><th>%s</th><th>%s</th>', Yum::t('Visitor'), Yum::t('Time of first Visit'), Yum::t('Time of last Visit'), Yum::t('Num of Visits'), Yum::t('Message')
        );

        foreach ($model->visits as $visit) {
            if (isset($visit->visitor)) {  //we need this in case a user quits
                printf('<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>', CHtml::link($visit->visitor->username, array(
                            '//profile/profile/view', 'id' => $visit->visitor_id)), date($format, $visit->timestamp_first_visit), date($format, $visit->timestamp_last_visit), $visit->num_of_visits, CHtml::link(Yum::t('Write a message'), array(
